...
namespace Discuz.Forum
{
    public class Navs
    {
        ...
        public static string GetNavigationString(int userid, int useradminid)
        {
            ...
            //分类
            DataTable dtab = GetSubForumListTable(0);
            StringBuilder sb = new StringBuilder();
            string fid  = "";
            string name = "";
            for (int i = 0; i < dtab.Rows.Count; i++)
            {
                fid = dtab.Rows[i]["fid"].ToString();
                name = dtab.Rows[i]["name"].ToString();
                sb.Append(@"<li><a id=""menu_5"" onMouseOver=""showMenu({'ctrlid':this.id});"" href=""./showforum-" + fid + @".aspx"">" + name + "</a></li>");
            }

            return mainNavList.ToArray()[0].ToString() + sb.ToString() + mainNavList.ToArray()[1].ToString() + mainNavList.ToArray()[2].ToString();
        }
